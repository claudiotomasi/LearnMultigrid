...
def new_neigh(C, F, C_neighs, F_neighs):
    new_C_neigh = {}

    # If 2 coarse nodes share a fine node, than they will be connected
    for i in range(0, len(C)):
        first = C[i]
        for j in range(i+1, len(C)):
            second = C[j]
            inters = np.intersect1d(C_neighs[first], C_neighs[second])
            inters = np.intersect1d(inters,F)
            if len(inters) > 0:
                if first in new_C_neigh:
                    if len(np.intersect1d(new_C_neigh[first],second)) == 0:
                        new_C_neigh[first] = np.append(new_C_neigh[first], second)
                else:
                    new_C_neigh[first] = second
                if second in new_C_neigh:
                    if len(np.intersect1d(new_C_neigh[second],first)) == 0:
                        new_C_neigh[second] = np.append(new_C_neigh[second], first)
                else:
                    new_C_neigh[second] = first
...
A = sio.loadmat(path+'stiff_2.mat')
A = A['A']


M = sio.loadmat(path+'mass_2.mat')
M = M['M']

rhs = sio.loadmat(path+'rhs_2.mat')
rhs = rhs['rhs']
print("data read")
...
new_C_ne = new_neigh(C, F, C_neigh, F_neigh)
...
bla = copy.copy(C_neigh)
re = list(range(0,len(conn)))
for el in C_neigh:
    values = C_neigh[el]
    rem = np.intersect1d(re, values)
    C_neigh[el] = rem[:2]
    re = list(set(re) - set(rem))