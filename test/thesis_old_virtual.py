...
def extract_patches(CC, _mat):
    ...
    for c_node in CC:
        ...
        print("coarse node: ", c_node, " on total: ", mat.shape[0])
        ii = ii + 1
...
def fill_B(_res, _idx_fill, total_size, mapping, CC):
    ...
    for j in range(0, len(_res)):
        ...
        if np.all(_B[col_B, node_coarse] == 0):
            _B[col_B, node_coarse] = pred[1:1 + len(col_B)]
        else:
            _B[col_B, node_coarse] = np.mean((_B[col_B, node_coarse],pred[1:1 + len(col_B)]))
        if np.all(_B[node, row_B] == 0):
            _B[node, row_B] = pred[7:7 + len(row_B)]
        else:
            _B[node, row_B] = np.mean((_B[node, row_B], pred[7:7 + len(row_B)]))
        ...
        for k in range(0, len(col_B)):
            ...
            if np.all(_B[col_B[k], true_where_coarse] == 0):
                _B[col_B[k], true_where_coarse] = pred[pos:pos + len(true_where_coarse)]
            else:
                _B[col_B[k], true_where_coarse] = np.mean((_B[col_B[k], true_where_coarse], pred[pos:pos + len(true_where_coarse)] ))
            pos = pos + 3
...
problem = path_matlab + "smile10_adapt.mat"
pp = scipy.io.loadmat(problem)
A = lil_matrix(pp['A'])
M = lil_matrix(pp['M'])
rhs = pp['rhs']
# L2 = pp['Q']
path_data = "/Users/claudio/Desktop/PhD/Tesi/dati_test/circle350/"
...
np.save(path_data + "patch.npy", patches)
np.save(path_data + "idx_fill.npy", idx_fill)
#
# # start = time.time()
