...
def extract_patches(CC, _mat):
    ...
    for c_node in CC:
        ...
        if additional > 0:
            ...
            st_single = time.time()
            patch, fill_idx = single_extraction(row.toarray()[0], where, mat, c_node, CC, node_M)
...
path_for_saving = "/Users/claudio/Desktop/PhD/Tesi/dati_test/smile_adapt/"

np.save(path_for_saving + "coarse.npy", C)
np.save(path_for_saving + "patch.npy", patches)
np.save(path_for_saving + "idx_fill.npy", idx_fill)
